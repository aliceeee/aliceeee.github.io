...
安装Docker，这里用Docker version 1.13.1
...
## CentOS GPU版 自定义镜像下pip安装
主机上支持GPU，在容器中使用paddlepaddle gpu
...
- 安装
  - 主机：
    ...
    - 按照上面检查基本环境
    - Install Docker 1.13
    - Install nvidia driver (参考下面)，硬件驱动
    - Install nvidia-docker-plugin，为了方便的run映射了gpu设备的容器
  - 容器镜像：
    - 按照上面检查基本环境和python
    - Install cuda+cudnn (参考下面)，提供高性能的GPU应用开发环境
    - Install paddlepaddle-gpu (参考下面)
